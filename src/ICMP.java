...
public class ICMP extends PacketStructure {
    ...
    */
    ...
    public void SetProperties(Packet packet) {
        ...
        properties.put(propertyLabels.get(1), icmp.code);
        if(icmp.type == 0 || icmp.type == 8
                || (icmp.type >= 13 && icmp.type <= 18)) {
            properties.put(propertyLabels.get(2),icmp.id);
            properties.put(propertyLabels.get(3), icmp.seq);
        }
        if(icmp.type == 5)
        ...
        if(icmp.type == 13 || icmp.type == 14){
            properties.put(propertyLabels.get(6), icmp.orig_timestamp);
            properties.put(propertyLabels.get(7), icmp.recv_timestamp);
            properties.put(propertyLabels.get(8), icmp.trans_timestamp);
        }
    }
}
